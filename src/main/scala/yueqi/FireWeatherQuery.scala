...
def heatMap(): Unit={
  ...
  val firedDistr = fireDF.filter($"FIRE_SIZE_CLASS"==="G").groupBy("STATE").count()
...
def queryFW(): Unit={                            
  ...
  //data and udf
  var fireDF0 = ssql.read.parquet("dataset-online/train/WildfireAll.parquet").select("OBJECTID", "STAT_CAUSE_CODE", "STATE", "FIRE_YEAR", "FIRE_SIZE_CLASS", "FIRE_SIZE", "DISCOVERY_DOY", "CONT_DOY")
  var fireDF = ssql.read.parquet("dataset-online/train/stratifiedSampleF2.parquet").select("OBJECTID", "STAT_CAUSE_CODE", "STATE", "FIRE_YEAR", "FIRE_SIZE_CLASS", "FIRE_SIZE", "DISCOVERY_DOY", "CONT_DOY")
  var weather = ssql.read.csv("dataset-online/train/stratifiedSampleW2.csv")
  var weatherDF = weather.toDF("OBJECTID","name","datetime","tempmax","tempmin","temp","feelslikemax","feelslikemin","feelslike","dew","humidity","precip","precipprob","precipcover","preciptype","snow","snowdepth","windgust","windspeed","winddir",
  "sealevelpressure","cloudcover","visibility","solarradiation","solarenergy","uvindex","severerisk")
...
//monthly precipitation, heat, and fire analysis
 ...
 //
  ...
  //percentage of small vs large fires in each state ties into hypothesis testing: are fire classes and states related
  val dallDF = fireDF0.groupBy("STATE").count().orderBy(col("count").desc).withColumnRenamed("count", "total fire counts")
  val dlargeDF = fireDF0.filter($"FIRE_SIZE_CLASS"==="F" || $"FIRE_SIZE_CLASS"==="G").groupBy("STATE").count().withColumnRenamed("count", "large fire counts").withColumnRenamed("STATE", "LSTATE")
  val joinLDF = dallDF.join(dlargeDF, dallDF("STATE")===dlargeDF("LSTATE")).select($"STATE", $"total fire counts", percentageudf($"large fire counts", $"total fire counts").alias("large fires percentage")).orderBy(col("large fires percentage").desc).show(20)
  //current weather data shows no correlation
  //hopefully past weather data will 
  // val precipAllDF = broadcast(combinedDF.filter("precip is not NULL")
  // .groupBy("month").agg(countDistinct("OBJECTIDW").alias("total fires"), round(sum("precip")).alias("total precipitation"), round(avg("heatidx")).alias("average heat index")).orderBy(col("month").asc)).show()
  ...
  .groupBy("month").agg(countDistinct("OBJECTIDW").alias("total small fires"), round(sum("precip")).alias("total precipitation"), round(avg("heatidx")).alias("average heat index")).orderBy(col("month").asc)).show()
  val precipLargeDF = broadcast(combinedDF.filter("precip is not NULL")
  .filter($"FIRE_SIZE_CLASS"==="F" || $"FIRE_SIZE_CLASS"==="G")
  .groupBy("month").agg(countDistinct("OBJECTIDW").alias("total large fires"), round(sum("precip")).alias("total precipitation"), round(avg("heatidx")).alias("average heat index")).orderBy(col("month").asc)).show()
